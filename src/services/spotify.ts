...
export const fetchSpotifyUserAccessToken = async () => {
  try {
    ...
    const data: { accessToken: string; expiryTime: string } = await response.json();
    console.log('@ data : ', data);

    localStorage.setItem(LOCAL_STORAGE_ACCESS_CODE, data.accessToken);
    localStorage.setItem(LOCAL_STORAGE_ACCESS_CODE_EXPIRY, data.expiryTime);
  } catch (error) {
